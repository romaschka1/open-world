...
public class AuthorizationResource {
  ...
  @PostMapping("refresh")
  public ResponseEntity<AuthorizationTokens> refreshAccessToken(@RequestBody AuthorizationTokens tokenRequest) {
    String refreshToken = tokenRequest.getRefreshToken();

    if (!jwtUtil.validateToken(refreshToken)) {
      throw new UserTokenError("Invalid refresh token");
    }

    String userId = jwtUtil.extractClaim(refreshToken, UserTokenClaimsEnum.id);
    User user = userRepository.findById(Long.valueOf(userId)).orElseThrow(() -> new BadRequestResourceError("Invalid token data"));
